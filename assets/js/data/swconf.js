  ...
  {% if site.pwa.cache.enabled %}
    ...
    interceptor: {
      ...
      urlPrefixes: [
        ...
        {%- comment -%}
          Stats API used by the visitor-count widget (_includes/visitor-count.html) — not the
          same endpoint as the pageview pixel above. Without this, the service worker (which only
          runs in production builds) cache-first-intercepts these requests and freezes the widget
          on whatever number it first saw, since the "this week" call's URL never changes.
        {%- endcomment -%}
        {% if site.analytics.goatcounter.id != nil and site.goatcounter_api_token %}
          'https://{{ site.analytics.goatcounter.id }}.goatcounter.com/api/',
        {% endif %}
      ]
    },
